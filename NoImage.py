...
def update_stock():
  ...
  for comic in comic_list:
    ...
    stock_string += "{}: {:.2f} - {:.0f}% of {:.2f} daily average sold\n".format(comic.name, comic.stock, progress, comic.average)
    total_comic += comic.stock

  stock_string += "\nTotal stock: {:.2f}".format(total_comic)
...
def restock_comics(comic):
  if comic.restock(amount.get()):
    ...
    action_feedback.set("Success! Total of {:.2f} restocked into {}".format(amount.get(), comic.name))
...
def sell_comics(comic):
  if comic.sell(amount.get()):
    ...
    action_feedback.set("Success! Total of {:.2f} sold from {}".format(amount.get(), comic.name))
...
# Set up Lists
comic_list = []
restock_messages = ["Well done, remember to always restock comics when we don't have enough to sell!", "You're making good progress!","Awesome! Make sure that you have more than or just the right amount of average daily comics sold."]
sell_messages = ["Think about where else you might be able to save some comics this week","You're doing well, but try to keep that spending under control","Tomorrow is another day for saving!"]
