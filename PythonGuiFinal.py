...
def restock_comics(comic):
  if comic.restock(amount.get()):
    ...
    action_feedback.set("Success! Total of {:.0f} copies of {} restocked.".format(amount.get(), comic.name))
...
def sell_comics(comic):
  ...
  if comic.sell(amount.get()):
    ...
    action_feedback.set("Success! Total of {:.0f} copies of {} sold.".format(amount.get(), comic.name))
    total_comic += amount.get()
    
  else:
    action_feedback.set("Not enough copies of {} or not a valid amount".format(comic.name))
